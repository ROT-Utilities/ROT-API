...
import { BlockRaycastOptions, EntityQueryOptions, EntityRaycastOptions, ExplosionOptions, Location, world } from 'mojang-minecraft';
...
setTickInterval(() => {
    ...
    if(bannedPlayers) for(let key in bannedPlayers) {
        if(bannedPlayers.hasOwnProperty(key) && bannedPlayers[key]?.bannedPlayer) if(bannedPlayers[key]?.unbanTime < today.getTime()) Server.bans.delete(key);
        else for(const player of world.getPlayers()) if(bannedPlayers[key]?.playerUUID === Server.player.getScore('ROTplayerUUID', player.name)) Server.runCommand(`kick "${player.name}" §r\n§cYou have been banned for §a${MS(bannedPlayers[key]?.length)}§c from this server at §b${bannedPlayers[key]?.date}${bannedPlayers[key]?.reason ? `\n§7Reason: §r${bannedPlayers[key]?.reason}` : ''}§7. You will be unbanned in ${MS(bannedPlayers[key]?.unbanTime - today.getTime())}`);
    }
}, 20);
const rides: any = {};
setTickInterval(() => {
    if(!ready) return;
    for(const player of world.getPlayers()) if(player.hasTag('is_sprinting') && player.hasTag('is_jumping') && !rides?.[player.nameTag]) {
    ...
    } else if(!player.hasTag('is_riding') && rides?.[player.nameTag]) delete rides[player.nameTag];
    const options = new EntityQueryOptions();
    options.type = 'minecraft:horse';
    for(const dim of ['overworld', 'nether', 'the end']) for(const entity of world.getDimension(dim).getEntities(options) ?? []) try { entity.runCommand(`testfor @a[r=2]`); } catch { entity.teleport(new Location(entity.location.x, -32767, entity.location.z), entity.dimension, 0, 0); };
}, 5);
world.events.entityCreate.subscribe(({ entity }) => {
    if(entity.id !== 'minecraft:horse' || entity.nameTag.startsWith('§6§l')) return;
