...
export class ServerPaper {
    ...
    broadcast(text: string, player?: string, use?: string, console?: boolean): runCommandReturn {
        if(!use || use == '') use = ''; else use = use.toUpperCase() + ' ';
        if(console === false) use = ''; else use = `§l§4${use}§7>>§r§7 `;
        if(player) if(player.startsWith('@')) null; else if(player.startsWith('"')) null; else player = `"${player}"`; else player = '@a';
    ...
    tBroadcast(text: string, player?: string, use?: string): runCommandReturn {
        if(!use) use = ''; else use = use.toUpperCase() + ' ';
        return this.runCommands([
    ...
    */
    public runCommand(command: string, dimension?: string): runCommandReturn {
        try {
