...
Server.command.register({
    ...
    if(cmdInfo) {
        if(cmdInfo.lister) {
            if(cmdInfo.admin && !Server.player.isAdmin(chatmsg.sender.name)) return Server.eBroadcast(config.prefix, chatmsg.sender.name, 'HELP');
            let commandsList = [];
        ...
        if(cmdInfo && cmdInfo.private) return Server.eBroadcast('I have no idea what you are talking about...', chatmsg.sender.name, 'HELP');
        if(cmdInfo && cmdInfo.admin && !Server.player.isAdmin(chatmsg.sender.name)) Server.eBroadcast('You think you have rights?', chatmsg.sender.name);
