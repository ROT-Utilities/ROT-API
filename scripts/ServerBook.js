...
class NewServerBook extends ServerPaper {
    ...
    _buildEvent() {
        world.events.beforeChat.subscribe(data => {
            ...
            const args = data.message.slice(config.prefix.length).trim().split(/\s+/), command = args.shift().toLowerCase(), getCommand = this.command.getAllRegistation().some(element => element.name === command || element.aliases && element.aliases.includes(command));
            if (!this.player.find(data.sender.name))
                return;
            if (!getCommand)
                return this.eBroadcast('That is not a command you can do on this server!', data.sender.name, 'ROT');
            this.command.getAllRegistation().forEach(element => {
                ...
                if (element?.admin && !this.player.isAdmin(data.sender.name))
                    return this.eBroadcast('You think you have rights?', data.sender.name, 'ROT');
                ...
                try {
                    ...
                    if (config.tellme)
                        this.broadcast(`Commad executed in §c${Date.now() - postCMDTime}§7 MS!`, data.sender.name, 'ROT');
                }
            });
        });
    }
