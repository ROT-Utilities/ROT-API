...
Server.command.register({
    ...
    if (cmdInfo) {
        if (cmdInfo.lister) {
            if (cmdInfo.admin && !Server.player.isAdmin(chatmsg.sender.name))
                return Server.eBroadcast(config.prefix, chatmsg.sender.name, 'HELP');
            let commandsList = [];
            for (let testCommand of Server.command.get()) {
                if ((Server.command.getRegistration(testCommand).category ? Server.command.getRegistration(testCommand).category.toLowerCase() : '') === cmdInfo.name.toLowerCase())
                    commandsList.push('§4§l' + testCommand + '§d - §5' + Server.command.getRegistration(testCommand).description);
            }
        ...
        if (cmdInfo && cmdInfo.private)
            return Server.eBroadcast('I have no idea what you are talking about...', chatmsg.sender.name, 'HELP');
        if (cmdInfo && cmdInfo.admin && !Server.player.isAdmin(chatmsg.sender.name))
            Server.eBroadcast('You think you have rights?', chatmsg.sender.name);
        let hI = `\n§l§cCommand: §l§5${config.prefix}§l§4${cmdInfo.name}§r\n`;
        if (cmdInfo.description)
            hI += `§l§cDescription§5:§r§5 ${cmdInfo.description}\n`;
        if (cmdInfo.aliases)
            hI += `§l§cAliases§5:\n §c§l${config.prefix}§r§5${cmdInfo.aliases.join(`§c,\n §l${config.prefix}§r§5`)}§r\n`;
        if (cmdInfo.category)
        ...
        if (cmdInfo.documentation) {
            ...
            if (cmdInfo.documentation.examples)
                hI += `\n§l§cExample(s)§5:\n §c§l${config.prefix}§r§5` + cmdInfo.documentation.examples.join(`§c,\n §l${config.prefix}§r§5`);
        ...
        return Server.broadcast('\n' + hI, chatmsg.sender.name, args[0] + ' HELP');
    }
    try {
        let page = isNaN(parseInt(args[0])) ? 1 : parseInt(args[0]), help = '', commandsList = Server.command.get().filter(command => {
            if (Server.command.getRegistration(command).private)
                return false;
            if (Server.command.getRegistration(command).admin && !Server.player.isAdmin(chatmsg.sender.name))
                return false;
            return true;
        });
        // @ts-ignore
        ...
        for (let command of result[page - 1]) {
            ...
            if (checkCommand.lister)
                help += `\n   §6§l<=-=-=-${checkCommand.name.toUpperCase()}=-=-=->§r\n`;
            else
                help += command, help += checkCommand.description ? `§d - §5${checkCommand.description}\n` : '';
        }
        Server.broadcast(help + `\n   §6§l<=-=-=-=-=-=-=-=-=->\n§cPage:§r §a${page}§d/§5${result.length}\nUse "§c${config.prefix}help§5" §d<Page Number> §5To see the next page`, chatmsg.sender.name, '', false);
    }
    catch (e) {
