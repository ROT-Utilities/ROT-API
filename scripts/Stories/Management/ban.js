...
/**
 * If you are in game wondering why the ban command won't show up... It's becuase it's a admin command
 */
Server.command.register({
    ...
    if (!restArgs || !restArgs[0].match(timeFormatRegex))
        return Server.eBroadcast(`${restArgs ? 'Invalid' : 'Missing'} ban length argument`, chatmsg.sender.name, 'BAN');
    const time = MS(restArgs[0].match(timeFormatRegex)[0]), reason = restArgs[0].replace(timeFormatRegex, '').replace(/^\s/, ''), today = new Date();
    Server.bans.set(player, {
        bannedPlayer: player,
    ...
    Server.broadcast(`Successfully banned §c${player}§7 for §c${restArgs[0].match(timeFormatRegex)[0]}§7${reason ? ` for "§c${reason}§7"` : ''}!`, chatmsg.sender.name, 'BAN');
});
Server.command.register({
    cancelMessage: true,
    name: 'banlist',
    ...
    if (args[0]) {
        ...
        if (player === chatmsg.sender.name)
            return Server.eBroadcast('You are not even banned...', chatmsg.sender.name, 'BANS');
        if (!Server.bans.has(player))
            return Server.eBroadcast(`No player with the name "§4${player}§c" is banned`, chatmsg.sender.name, 'BANS');
    ...
    if (!bannedPlayers)
        return Server.eBroadcast('There are currently NO banned players in this server!', chatmsg.sender.name, 'BANS');
    for (let key in bannedPlayers)
        if (bannedPlayers.hasOwnProperty(key) && bannedPlayers[key]?.bannedPlayer)
            allBannedplayers.push(bannedPlayers[key]?.bannedPlayer + '§7 - §e' + MS(bannedPlayers[key]?.unbanTime - today));
...
Server.command.register({
    ...
    if (!args.join(' ').match(/(?<=^")([^"]+)(?=")/))
        return Server.eBroadcast('Type the player name in quotation marks for the first argument!', chatmsg.sender.name, 'UNBAN');
    const player = args.join(' ').match(/(?<=^")([^"]+)(?=")/)[0];
    if (player === chatmsg.sender.name)
        return Server.eBroadcast('You are not even banned...', chatmsg.sender.name, 'UNBAN');
    if (!Server.bans.has(player))
        return Server.eBroadcast(`No player with the name "§4${player}§c" is banned`, chatmsg.sender.name, 'UNBAN');
