...
Server.command.register({
    ...
    if (!args.join(' ').match(/(?<=^")([^"]+)(?=")/))
        return Server.eBroadcast('Type the player name in quotation marks for the first argument!', chatmsg.sender.name, 'RANKS');
    const player = args.join(' ').match(/(?<=^")([^"]+)(?=")/)[0];
    if (!Server.player.find(player))
        return Server.eBroadcast('That player is not real!', chatmsg.sender.name, 'RANKS');
    const restArgs = args.join(' ').match(new RegExp(`(?<=^"${player}"\\s).+`)), add = ['add', 'plus', 'more', 'make', 'put'], list = ['list', 'all', 'tell', 'for'], remove = ['remove', 'destroy', 'bomb', 'yeet'], addRegex = /(add|plus|more|make|put)/, revRegex = /(remove|destory|bomb|yeet)/;
    try {
        let args2 = restArgs[0].match(/(add|plus|more|make|put|list|all|tell|for|remove|destory|bomb|yeet)/)[0];
        if (add.includes(args2)) {
            if (restArgs[0].match(addRegex)[0] === '')
                return Server.eBroadcast('Type the rank you want to give!', chatmsg.sender.name, 'RANKS');
            Server.player.fetch(player).addTag('rank:' + restArgs[0].replace(addRegex, '').replace(/^\s/, ''));
            let ranks = getRanks(player).split('§r§7, ').length;
        ...
        if (!restArgs || list.includes(args2)) {
            ...
            return Server.broadcast(`§c${player}§7 has a total of §4${ranks.length}§7 rank${ranks.length === 0 || ranks.length >= 2 ? 's' : ''}! Here is a list of them:\n${ranks.join('§r§7,\n')}§r§7.`, chatmsg.sender.name, 'RANKS');
        }
        if (remove.includes(args2)) {
            if (restArgs[0].match(revRegex)[0] === '')
                return Server.eBroadcast('Type the rank you want to give!', chatmsg.sender.name, 'RANKS');
        ...
        Server.eBroadcast('You done goofed up!', chatmsg.sender.name, 'RANKS');
    }
});
